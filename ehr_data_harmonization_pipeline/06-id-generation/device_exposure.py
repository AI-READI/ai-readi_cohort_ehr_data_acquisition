...
if dbutils.widgets.get("site")  == 'UAB':
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_device_exposure AS

        WITH join_conflict_id AS (
            SELECT 
                d.*
                , COALESCE(lookup.collision_bits, 0) as collision_index
            FROM {databaseName}.04_device_exposure d
            LEFT JOIN {databaseName}.05_device_exposure lookup
        ...
        -- and global_id.device_exposure_start_datetime= v.visit_start_datetime 
        -- and global_id.device_exposure_end_datetime= v.visit_end_datetime
    """)
else:
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_device_exposure AS
...
timestamp_comment.comment(spark,databaseName,'06_device_exposure')
