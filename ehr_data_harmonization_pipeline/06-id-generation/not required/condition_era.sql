...
CREATE TABLE IDENTIFIER(:databaseName ||'.06_condition_era') AS

WITH join_conflict_id AS (
    SELECT 
        d.*
        , COALESCE(lookup.collision_bits, 0) as collision_index
    FROM IDENTIFIER(:databaseName ||'.04_condition_era') d
    LEFT JOIN IDENTIFIER(:databaseName ||'.05_condition_era') lookup
...
-- MAGIC timestamp_comment.comment(spark,dbname,'06_condition_era')
