...
CREATE TABLE IDENTIFIER(:databaseName ||'.06_visit_detail') AS

WITH join_conflict_id AS (
    SELECT 
        d.*
        , COALESCE(lookup.collision_bits, 0) as collision_index
    FROM IDENTIFIER(:databaseName ||'.04_visit_detail') d
    LEFT JOIN IDENTIFIER(:databaseName ||'.05_visit_detail') lookup
...
global_visit_detail as ( 
    ...
    LEFT JOIN IDENTIFIER(:databaseName ||'.06_care_site') care
    ON global_id.site_care_site_id = care.site_care_site_id
    LEFT JOIN IDENTIFIER(:databaseName ||'.06_provider') prov
    ON global_id.site_provider_id = prov.site_provider_id
    LEFT JOIN IDENTIFIER(:databaseName ||'.06_visit_occurrence') visit
...
SELECT gvd.*
    ...
    LEFT JOIN IDENTIFIER(:databaseName ||'.06_visit_occurrence') enc
...
-- MAGIC timestamp_comment.comment(spark,dbname,'06_visit_detail')
-- MAGIC
-- MAGIC
