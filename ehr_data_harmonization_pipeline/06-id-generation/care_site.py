...
spark.sql(f"""
    CREATE OR REPLACE TABLE {databaseName}.06_care_site AS

    WITH join_conflict_id AS (
        SELECT 
            d.*
            , COALESCE(lookup.collision_bits, 0) as collision_index
        FROM {databaseName}.04_care_site d
        LEFT JOIN {databaseName}.05_care_site lookup
...
timestamp_comment.comment(spark,databaseName,'06_care_site')
