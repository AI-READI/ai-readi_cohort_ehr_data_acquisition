...
if dbutils.widgets.get("site")  == 'UAB':
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_observation AS

        WITH join_conflict_id AS (
            SELECT 
                d.*
                , COALESCE(lookup.collision_bits, 0) as collision_index
            FROM {databaseName}.04_observation d
            LEFT JOIN {databaseName}.05_observation lookup
        ...
        -- and global_id.observation_datetime= v.visit_start_datetime 
    """)
else: 
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_observation AS
...
timestamp_comment.comment(spark,databaseName,'06_observation')
