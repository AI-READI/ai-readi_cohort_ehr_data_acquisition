...
if dbutils.widgets.get("site")  == 'UAB':
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_drug_exposure AS

        WITH join_conflict_id AS (
            SELECT 
                d.*
                , COALESCE(lookup.collision_bits, 0) as collision_index
            FROM {databaseName}.04_drug_exposure d
            LEFT JOIN {databaseName}.05_drug_exposure lookup
        ...
        -- and global_id.drug_exposure_start_datetime= v.visit_start_datetime 
        -- and global_id.drug_exposure_end_datetime= v.visit_end_datetime
    """)
else:
    spark.sql(f"""
        CREATE OR REPLACE TABLE {databaseName}.06_drug_exposure AS
...
timestamp_comment.comment(spark,databaseName,'06_drug_exposure')
