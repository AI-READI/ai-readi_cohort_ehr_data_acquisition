...
spark.sql(f"""
    CREATE OR REPLACE TABLE {databaseName}.06_provider AS

    WITH join_conflict_id AS (
        SELECT 
            d.*
            , COALESCE(lookup.collision_bits, 0) as collision_index
        FROM {databaseName}.04_provider d
        LEFT JOIN {databaseName}.05_provider lookup
...
timestamp_comment.comment(spark,databaseName,'06_provider')
