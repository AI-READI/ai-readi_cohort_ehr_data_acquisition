...
def union_many_dataframes(*dfs):
    """
    Union multiple dataframes together.
    Replacement for Foundry's D.union_many() function.
    """
    if not dfs:
        return None
...
def clean_free_text_cols(input_df, domain, p_key):
    domain = domain.lower()
    ...
    if phi_cols:
        ...
        final_nulled_rows_df = union_many_dataframes(*nulled_rows_dfs)
...
def do_pre_clean(
    ...
    foundry_df, nulled_rows_df = clean_free_text_cols(foundry_df, domain, p_key)
    ...
    if nulled_rows_df:
        ...
        nulled_rows_df.write.mode("overwrite").saveAsTable(nulled_rows)
    ...
    if len(bad_rows_dfs) > 0:
        removed_rows_df = union_many_dataframes(*bad_rows_dfs)
        ...
        removed_rows_df.write.mode("overwrite").saveAsTable(removed_rows)

    else: 
        #removed_rows.write_dataframe(ctx.spark_session.createDataFrame([], schema=foundry_df.schema))
        schema_string = ", ".join([f"`{field.name}` {field.dataType.simpleString()}" for field in foundry_df.schema.fields])
        spark.sql(f"CREATE OR REPLACE TABLE {removed_rows} ({schema_string})")
    ...
    foundry_df.write.mode("overwrite").saveAsTable(processed)
