...
def apply_schema(df, schema_dict, include_payload=True):
    """
    Apply schema transformations to a DataFrame with comprehensive error handling
    """
    # Step 1: Convert empty strings to null values first
    exprs = [F.when((F.col(x) != "") & (F.upper(F.col(x)) != "NULL"), F.col(x)).otherwise(None).alias(x) for x in df.columns]
    df = df.select(*exprs)

    # Step 2: Make sure all column names are uppercase
    for original_col_name in df.columns:
        df = df.withColumnRenamed(original_col_name, original_col_name.upper())
    input_cols = df.columns

    # Step 3: Handle PERSON_ID conversion before type casting
    if "PERSON_ID" in input_cols and "PERSON_ID" in schema_dict:
        target_type = schema_dict["PERSON_ID"]
        if target_type == T.LongType():
            print("Converting PERSON_ID from string to integer...")
            df = df.withColumn(
                "PERSON_ID",
                F.when(
                    # If it's already numeric (only digits), cast directly
                    F.col("PERSON_ID").rlike("^\\d+$"),
                    F.expr("try_cast(PERSON_ID as BIGINT)")
                ).when(
                    # If it contains letters or mixed, hash it to get consistent integer
                    F.col("PERSON_ID").isNotNull(),
                    F.abs(F.hash(F.col("PERSON_ID"))).cast(T.LongType())
                ).otherwise(
                    F.lit(None).cast(T.LongType())
                )
            )

    # Step 4: Process each column according to schema
    for col_name, col_type in schema_dict.items():
        if col_name in input_cols:
            # Skip PERSON_ID since we already handled it above
            if col_name == "PERSON_ID" and col_type == T.LongType():
                continue
                
            # Handle date/timestamp columns
            if col_type == T.DateType() or col_type == T.TimestampType():
                # Handle Unix timestamps (only if the value is purely numeric)
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^\\d+$"),  # Only pure numbers
                        F.from_unixtime(df[col_name].cast(T.LongType()))
                ...
                # Handle simple date format: 4/27/2018
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^\\d{1,2}\\/\\d{1,2}\\/\\d{4}$"),
                        F.to_date(F.col(col_name), 'MM/dd/yyyy')
                    ).otherwise(df[col_name])
                )

                # Handle date with zero time: 4/27/2018 0:00
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^\\d{1,2}\\/\\d{1,2}\\/\\d{4} \\d{1,2}:\\d{2}$"),
                        F.to_date(F.col(col_name), 'MM/dd/yyyy H:mm')
                    ).otherwise(df[col_name])
                )
            
            # Step 5: Cast to the target type with safe handling for numeric types
            if col_type == T.LongType():
                # Use try_cast for BIGINT to handle malformed data like '50379226.0'
                # df = df.withColumn(col_name, F.expr(f"try_cast(`{col_name}` as BIGINT)"))
                df = df.withColumn(col_name, F.expr(f"CAST(CAST(`{col_name}` AS DOUBLE) AS BIGINT)"))
            elif col_type == T.IntegerType():
                # Use try_cast for INT to handle malformed data
                df = df.withColumn(col_name, F.expr(f"try_cast(`{col_name}` as INT)"))
            elif col_type == T.DoubleType():
                # Use try_cast for DOUBLE to handle malformed data
                df = df.withColumn(col_name, F.expr(f"try_cast(`{col_name}` as DOUBLE)"))
            elif col_type == T.FloatType():
                # Use try_cast for FLOAT to handle malformed data
                df = df.withColumn(col_name, F.expr(f"try_cast(`{col_name}` as FLOAT)"))
            else:
                # For other types (String, Boolean, etc.), use regular cast
    ...
    # Step 6: Reorder columns according to schema
    col_list = list(schema_dict.keys())
    if include_payload and "PAYLOAD" in df.columns:
...
def apply_schema_debug(df, schema_dict, include_payload=True):
    print("=== APPLY_SCHEMA DEBUG START ===")
    
    # Convert empty strings to null values first
    print("Step 1: Converting empty strings to null...")
    exprs = [F.when((F.col(x) != "") & (F.upper(F.col(x)) != "NULL"), F.col(x)).otherwise(None).alias(x) for x in df.columns]
    df = df.select(*exprs)

    # Make sure all column names are uppercase
    print("Step 2: Converting column names to uppercase...")
    for original_col_name in df.columns:
        df = df.withColumnRenamed(original_col_name, original_col_name.upper())
    input_cols = df.columns

    # Handle PERSON_ID conversion before type casting
    if "PERSON_ID" in input_cols and "PERSON_ID" in schema_dict:
        target_type = schema_dict["PERSON_ID"]
        if target_type == T.LongType():
            print("Step 3: Converting PERSON_ID from string to integer...")
            
            # Check what's in PERSON_ID before conversion
            print("PERSON_ID values that look like dates:")
            date_like_person_ids = df.filter(F.col("PERSON_ID").rlike("^\\d{4}-\\d{2}-\\d{2}$")).count()
            if date_like_person_ids > 0:
                print(f"WARNING: Found {date_like_person_ids} date-like values in PERSON_ID!")
                df.filter(F.col("PERSON_ID").rlike("^\\d{4}-\\d{2}-\\d{2}$")).select("PERSON_ID").distinct().show()
            ...
            df = df.withColumn(
                ...
                F.when(
                    # If it's already numeric (only digits), cast directly
                    F.col("PERSON_ID").rlike("^\\d+$"),
                    F.expr("try_cast(PERSON_ID as BIGINT)")  # Use try_cast here too
                ).when(
                    # If it contains letters or mixed, hash it to get consistent integer
                    F.col("PERSON_ID").isNotNull(),
                    F.abs(F.hash(F.col("PERSON_ID"))).cast(T.LongType())
            ...
            print("PERSON_ID conversion completed.")

    # Continue with the rest of the schema application...
    print("Step 4: Processing each column according to schema...")
    for col_name, col_type in schema_dict.items():
        print(f"\n--- Processing {col_name} (target type: {col_type}) ---")
        
        if col_name in input_cols:
            # Skip PERSON_ID since we already handled it above
            if col_name == "PERSON_ID" and col_type == T.LongType():
                print(f"Skipping {col_name} - already processed")
                continue
            
            # Check what's actually in this column before processing
            if col_name in ["CONDITION_START_DATE", "CONDITION_END_DATE"]:
                print(f"Sample values in {col_name} before processing:")
                df.select(col_name).distinct().limit(5).show()
                
            try:
                if col_type == T.DateType() or col_type == T.TimestampType():
                    print(f"Processing {col_name} as DATE/TIMESTAMP")
                    ...
                    print(f"Date processing completed for {col_name}")
                
                # Cast to the target type
                print(f"Casting {col_name} to {col_type}")
                df = df.withColumn(col_name, df[col_name].cast(col_type))
                print(f"Successfully cast {col_name}")
                
                # Show sample values after casting for date columns
                if col_name in ["CONDITION_START_DATE", "CONDITION_END_DATE"]:
                    print(f"Sample values in {col_name} after processing:")
                    df.select(col_name).distinct().limit(5).show()

            except Exception as e:
                print(f"ERROR processing column {col_name}: {str(e)}")
                print(f"Column type was: {col_type}")
                # Show some sample data for debugging
                try:
                    print(f"Sample values in {col_name} when error occurred:")
                    df.select(col_name).distinct().limit(5).show()
                except:
                    pass
                raise e
            ...
            # Populate column with null if it isn't in input table
            print(f"Adding missing column {col_name} with null values")
            df = df.withColumn(col_name, F.lit(None).cast(col_type))
    ...
    print("Step 6: Converting column names to lowercase...")
    for original_col_name in df.columns:
        df = df.withColumnRenamed(original_col_name, original_col_name.lower())

    print("=== APPLY_SCHEMA DEBUG END ===")
    return df

# In schema.py, modify the apply_schema function:
def apply_schema_org(df, schema_dict, include_payload=True):
    # Convert empty strings to null values first
    exprs = [F.when((F.col(x) != "") & (F.upper(F.col(x)) != "NULL"), F.col(x)).otherwise(None).alias(x) for x in df.columns]
    df = df.select(*exprs)
        ...
        if target_type == T.LongType():
            ...
            df = df.withColumn(
                ...
                F.when(
                    # If it's already numeric (only digits), cast directly
                    F.col("PERSON_ID").rlike("^\\d+$"),
                    F.col("PERSON_ID").cast(T.LongType())
                ).when(
                    # If it contains letters or mixed, hash it to get consistent integer
                    F.col("PERSON_ID").isNotNull(),
                    F.abs(F.hash(F.col("PERSON_ID"))).cast(T.LongType())
...
# Data types are not used in step 8 EHR pipeline
omop_required = {
    "care_site": {
        "CARE_SITE_ID": T.LongType(),
    },
