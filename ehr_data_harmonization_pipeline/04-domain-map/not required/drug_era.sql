...
CREATE TABLE IDENTIFIER(:databaseName ||'.04_drug_era')
...
(
	...
	FROM omop_vocab.concept_ancestor ca
		INNER JOIN omop_vocab.concept c ON ca.ancestor_concept_id = c.concept_id
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
	SELECT
		...
		, data_partner_id
		, payload
	FROM IDENTIFIER(:databaseName ||'.04_drug_exposure') d
...
-- MAGIC timestamp_comment.comment(spark,dbname,'04_drug_era')
-- MAGIC
